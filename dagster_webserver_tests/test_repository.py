...
def define_empty_job():
    return dg.JobDefinition(
        name="empty_job", graph_def=dg.GraphDefinition(name="empty_graph")
    )
...
def define_with_resources_job():
    ...
    adder_job = my_graph.to_job(
        name="adder_job", resource_defs={"modifier": adder_resource}
    )
    multer_job = my_graph.to_job(
        name="multer_job", resource_defs={"modifier": multer_resource}
    )
    ...
    multi_job = my_graph.to_job(
        "multi_job",
        resource_defs={"modifier": adder_resource},
...
def test_asset_value_loader_with_config():
    ...
    value = repo.load_asset_value(
        dg.AssetKey("asset1"), resource_config=resource_config
    )
    assert value == 5
...
def test_asset_value_loader_with_resources():
    ...
    def repo():
        return dg.with_resources(
            [asset1],
            resource_defs={"io_manager": my_io_manager, "io_resource": io_resource},
        )
...
def test_asset_value_loader_with_metadata():
    ...
    def repo():
        return dg.with_resources(
            [asset1, asset2], resource_defs={"io_manager": my_io_manager}
        )
